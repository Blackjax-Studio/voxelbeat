...
export default function MusicPlayer({
    ...
    // Reset avatar error when avatarUrl changes and check if image can load
    useEffect(() => {
        setAvatarError(false);

        // Check if the image loads after a short delay
        if (avatarUrl) {
            const timer = setTimeout(() => {
                if (imgRef.current) {
                    // Check if image actually loaded (naturalWidth will be 0 if it failed)
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            // We have 6 bars (3 for mobile list, 3 for desktop list)
            // Map them to different frequency ranges: Low (bass), Mid, High-Mid
            const indices = [4, 12, 20]; // Adjust based on fftSize and typical music
    ...
    useEffect(() => {
        // Change visualization style to a random one when track or artist changes
        const styles: VisStyle[] = ['Circular', 'Bars', 'Waveform', 'Nebula'];

        // Pick a style different from the current one to ensure a change is visible
        const otherStyles = styles.filter(s => s !== visStyle);
        const nextStyle = otherStyles[Math.floor(Math.random() * otherStyles.length)];

        console.log(`[DEBUG_LOG] Randomly changing visualization style to: ${nextStyle} for artist: ${artistName}, track index: ${currentTrackIndex}`);
    ...
    }, [currentTrackIndex, tracks, artistName, setVisStyle]);
                ...
                <div className={`flex flex-col xl:w-[440px] xl:shrink-0 w-full`}>
                    ...
                    repeating-linear-gradient(-45deg, rgba(255,255,255,0.05) 0px, rgba(255,255,255,0.05) 1px, transparent 1px, transparent 20px)`
                      ...
                      ${isPlaying ? 'animate-spin' : ''} bg-yellow-400 backdrop-blur-md border border-white/20 shadow-xl group-hover:shadow-yellow-400/20 transition-all overflow-hidden`}
                                ...
                                <div className="flex flex-col items-center text-center">
                                    ...
                                    {((tracks[currentTrackIndex]?.tags && Array.isArray(tracks[currentTrackIndex].tags)) ||
                                      (typeof (tracks[currentTrackIndex] as any)?.tags === 'string' && (tracks[currentTrackIndex] as any).tags)) && (
                                        <div className="flex flex-wrap items-center justify-center gap-1 mt-1 xs:mt-1.5 max-w-full overflow-hidden">
                                            {(() => {
                                                const allTags = Array.isArray(tracks[currentTrackIndex].tags)
                                                    ? tracks[currentTrackIndex].tags
                                                    : (tracks[currentTrackIndex] as any).tags.split(',').filter(Boolean);
                    ...
                    <div className="px-3 xs:px-5 pt-3 xs:pt-4 pb-4 xs:pb-5 flex flex-col gap-3 xs:gap-4">
                        ...
                        <div className="flex items-center justify-between px-1 xs:px-2">
                            ...
                            {/* Track list toggle */}
                            <div className={`group/tooltip relative flex items-center justify-center xl:hidden`}>
                        ...
                        {/* Mobile Track list (shows when toggled on mobile, or always hidden on desktop if we are in two-column mode) */}
                        {showTrackList && (
                            <div className={`flex flex-col gap-0.5 xs:gap-1 pt-2 border-t border-white/5 max-h-40 xs:max-h-48 overflow-y-auto custom-scrollbar xl:hidden`}>
                                {tracks.map((track, i) => (
                        ...
                        ${i === currentTrackIndex ? `bg-gradient-to-br ${trackColors[i % trackColors.length]} ring-1 ring-white/30` : 'bg-white/10'}`}>
                                        ...
                                        <div className="flex-1 flex flex-col min-w-0">
                                            ...
                                            {track.description && (
                                                <p className="text-white/40 text-[8px] xs:text-[9px] leading-tight italic">
                                                    {track.description}
                                                </p>
                                            )}
                                        </div>
                ...
                <div className="hidden xl:flex xl:w-[440px] xl:shrink-0 flex-col gap-4 p-6 border-l border-white/5 bg-black/10">
                    ...
                    ${i === currentTrackIndex ? `bg-gradient-to-br ${trackColors[i % trackColors.length]} ring-1 ring-white/30` : 'bg-white/10'}`}>
                                    ...
                                    <div className="flex-1 flex flex-col min-w-0">
                                        ...
                                        {track.description && (
                                            <p className="text-white/40 text-[11px] leading-tight italic mt-0.5">
                                                {track.description}
                                            </p>
                                        )}
                                    </div>
